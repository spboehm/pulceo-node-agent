...
- First, deploy pulceo on another
  machine [pulceo-resource-manager#quickstart](https://github.com/spboehm/pulceo-resource-manager?tab=readme-ov-file#quickstart-try-locally)
- The machine that is running the pulceo-node-agent must have a public IP address with a fully qualified domain name (
  FQDN)
...
```bash
bash <(curl -s https://raw.githubusercontent.com/spboehm/pulceo-node-agent/v1.3.0/bootstrap-pulceo-node-agent.sh)
```
